...
make_test_tcp_4down () {
local test_name="tcp_4down"

# Getting device ID
current_device_id="$host"
    
echo "TCP_4DOWN test was started"
...
mkdir -p "$report_dir"
    ...
    flent "$test_name" --swap-up-down --data-dir="$data_dir" --output="$data_dir/netperf_output" --length="$length" --step-size=0.5 --host="$host" --extended-metadata --socket-stats --title-extra="EvilWireless wifi $test_name test for $host"
